...
def process_batch_for_llava(batch, processor, device, llava_prompt_template, few_shot_images=None, metadata_strings=None):
    """
    ...
    if metadata_strings and '{metadata}' in llava_prompt_template:
        # Use metadata if available and template expects it
        prompts = [llava_prompt_template.format(text=t, metadata=meta) for t, meta in zip(texts, metadata_strings)]
    else:
    ...
    try:
        # Handle different processor types and their expected input formats
        processor_type = type(processor).__name__
        logger.info(f"Processing with processor type: {processor_type}")
        
        if processor_type == 'Qwen2_5_VLProcessor':
            # Special handling for SpaceQwen2.5-VL-3B-Instruct
            logger.info("Using SpaceQwen2.5-VL-3B specific processing")
            try:
                # SpaceQwen expects different input format
                inputs = processor(
                    text=prompts,
                    images=all_images_for_processor,
                    return_tensors="pt",
                    padding=True,
                    truncation=True
                )
            except Exception as e:
                logger.error(f"SpaceQwen processing failed: {e}")
                # Fallback: try with separate text and image processing
                if hasattr(processor, 'image_processor') and hasattr(processor, 'tokenizer'):
                    logger.info("Trying fallback processing for SpaceQwen")
                    text_inputs = processor.tokenizer(prompts, return_tensors="pt", padding=True, truncation=True)
                    image_inputs = processor.image_processor(all_images_for_processor, return_tensors="pt")
                    inputs = {**text_inputs, **image_inputs}
                else:
                    raise e
        elif hasattr(processor, 'image_processor'):
            # For processors with image_processor but unified text processing (standard LLaVA)
            try:
                # Try unified processing first
                inputs = processor(text=prompts, images=all_images_for_processor, return_tensors="pt", padding=True)
            except Exception as e:
                logger.warning(f"Unified processing failed: {e}")
                # Fallback to separate processing
                if hasattr(processor, 'tokenizer'):
                    text_inputs = processor.tokenizer(prompts, return_tensors="pt", padding=True)
                    image_inputs = processor.image_processor(all_images_for_processor, return_tensors="pt")
                    inputs = {**text_inputs, **image_inputs}
                else:
                    raise e
        else:
            # For unified processors - handle patch_size issue
            try:
                inputs = processor(text=prompts, images=all_images_for_processor, return_tensors="pt", padding=True)
            except TypeError as e:
                if "unsupported operand type(s) for //: 'int' and 'NoneType'" in str(e):
                    # Fix for models with None patch_size
                    logger.warning("Detected None patch_size in LLaVA processor. Attempting to fix...")
                    if hasattr(processor, 'image_processor') and hasattr(processor.image_processor, 'patch_size'):
                        # Set a default patch_size if it's None
                        if processor.image_processor.patch_size is None:
                            processor.image_processor.patch_size = 14  # Default for most LLaVA models
                            logger.info("Set patch_size to 14 for LLaVA processor")
                    inputs = processor(text=prompts, images=all_images_for_processor, return_tensors="pt", padding=True)
                else:
                    raise e

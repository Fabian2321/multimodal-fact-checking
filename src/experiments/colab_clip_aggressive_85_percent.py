# --- Colab-compatible aggressive CLIP Script for 85%+ Accuracy ---
# Before execution: !pip install transformers torch pillow pandas scikit-learn matplotlib seaborn nltk opencv-python
...
def create_advanced_image_crops(image: Image.Image, num_crops: int = 5) -> List[Image.Image]:
    """Creates extended image crops for maximum robustness"""
...
def apply_image_augmentation(image: Image.Image) -> Image.Image:
    ...
    # Convert to numpy for simple augmentation
...
def calculate_comprehensive_metrics(y_true, y_pred, similarities, threshold):
    ...
    print("\n" + "="*60)
    print("AGGRESSIVE CLIP EXPERIMENT - COMPLETE METRICS")
    print("="*60)
    print(f"Setup:")
...
def plot_aggressive_results(metrics, threshold_analysis):
    """Creates enhanced visualizations of aggressive results"""
...
def main():
    ...
    NUM_CROPS = 5  # Extended multi-crop
    ...
    for name, threshold in thresholds_to_test:
        ...
        if name == 'roc_threshold':
            ...
            threshold_analysis['pr_accuracy'] = accuracy
        elif name == 'ba_threshold':
            threshold_analysis['ba_accuracy'] = accuracy
        elif name == 'custom_threshold':
            threshold_analysis['custom_accuracy'] = accuracy
    ...
    plot_aggressive_results(metrics, threshold_analysis)

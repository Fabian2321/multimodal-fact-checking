...
# BLIP2 with RAG and Few-Shot Learning for better performance
...
def create_rag_context(text: str, knowledge_base: List[Dict], top_k: int = 3) -> str:
    """Creates RAG context based on text similarity"""
    ...
    # Vectorize all documents and query
...
def create_few_shot_examples() -> str:
    """Creates few-shot examples for consistent answers"""
    examples = """
...
def create_enhanced_prompt(text: str, rag_context: str = "") -> str:
    """Creates enhanced prompt with RAG and few-shot"""
...
def parse_blip2_response_enhanced(response: str) -> float:
    """Enhanced BLIP2 response parsing with RAG context"""
    ...
    if fake_count > real_count:
        ...
        # Neutral based on length and words
...
class EnhancedBLIP2Handler:
    def __init__(self, use_rag: bool = True, use_few_shot: bool = True):
        """Enhanced BLIP2 Handler with RAG and few-shot"""
    ...
    def predict_enhanced(self, text: str, image: Image.Image) -> Dict[str, Any]:
        """Enhanced prediction with RAG and few-shot"""
        ...
        # Only original image (no multi-crop for speed)
        crops = [image]
            ...
            try:
                ...
                with torch.no_grad():
                    outputs = self.model.generate(
                        **inputs,
                        max_new_tokens=100,  # Longer answers for analysis
                        num_beams=3,
                        do_sample=False,
                        temperature=1.0,
...
def calculate_enhanced_metrics(y_true, y_pred, scores, threshold):
    """Calculates metrics for enhanced BLIP2"""
    ...
    print("\n" + "="*60)
    print("ENHANCED BLIP2 + RAG + FEW-SHOT - FULL METRICS")
    print("="*60)
    print(f"Setup:")
...
def plot_enhanced_results(metrics):
    """Creates visualizations for enhanced BLIP2"""
...
def main():
    """Enhanced BLIP2 experiment with RAG and few-shot"""
    
    # Parameters
    CSV_FILE = "test_balanced_pairs_clean.csv"
    NUM_SAMPLES = 50  # Fewer samples for longer prompts
    ...
    for idx, row in df.iterrows():
        if idx % 5 == 0:  # More frequent updates due to longer prompts
